...
async function getTagPageData(slug: string, page: number) {
  try {
    const [tagMeta, total] = await Promise.all([
...
export default async function TagPage({ params, searchParams }: TagPageProps) {
  ...
  const totalPages = Math.max(1, Math.ceil(data.total / TAG_PAGE_SIZE))
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              {data.total} {data.total === 1 ? 'article' : 'articles'} tagged with #{tag.slug}
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 pb-16">
          {posts.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-muted-foreground text-lg">No articles with this tag yet.</p>
...
export async function generateMetadata({ params }: TagPageProps) {
  ...
  if (!tag) {
    return {
      title: 'Tag Not Found - ByteBrief'
    }
  }

  return {
    title: `${tag.name} Articles - ByteBrief`,
    description: `Browse all posts tagged with ${tag.name} on ByteBrief.`,
  }
}
